defmodule ConnectNode do
  ...
  @default_node ["master@mickeyoh.local", "chocolatechip", "rasp@nerves.local"]
  @hostname "mickeyoh.local"

  def start_link(node_option \\ @default_node) do
    GenServer.start_link(__MODULE__, node_option, name: __MODULE__)
  end

  def init(node_option) do
    Logger.info("activated Connect Node")
  ...
  def get_ipaddr_wlan0() do
    ...
    #   [list_int_wlan0_addr] ->
    #     ipdt = list_int_wlan0_addr
    #     |> (fn {_, list_settings} -> list_settings end).()
    #     |> Enum.find(  fn map -> map[:family] == :inet end)

    #     case ipdt do
    #       nil -> nil
    #       ipdt -> Map.get(ipdt, :address)
    #              |> VintageNet.IP.ip_to_string()
